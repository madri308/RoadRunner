...
public:
    ...
    int CONFIGURATIONS_QUANTITY = 15;                   //Cantidad de configuraciones
    std::vector<int> POPULATION_LENGTH_RANGE{0,50};     //Rango de poblacion.
    std::vector<int> SENSORS_QUANTITY_RANGE{1,10};      //Rango de sensores.
    std::vector<int> GENERATION_QUANTITY_RANGE{0,10000};//Rango de generaciones.
    ...
    std::vector<int> SECURITY_DISTANCE{2,20};           //Distancia antes del limite.
    int individualSize = 10;                            //mitad del individuo.
    float MUTATE_PERCENTAGE = 3.0f;                     //Porcentaje de mutacion
    float MUTATION = 5;                                //Valor de mutacion
};
